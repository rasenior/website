...
We had a new paper out this week in [*Nature Climate Change*](https://www.nature.com/articles/s41558-019-0529-2), looking at the combined threats of forest loss and climate change in the tropics. 

The work combines global layers of forest cover [@hansen_high-resolution_2013] with current and future mean annual temperature [@hijmans_very_2005], to assess how connected tropical forests are to analogous future climates. The thinking is that tropical forest species sensitive to climate change (which is thought to be many of them) will need to move towards the poles or -- more likely -- uphill, to stay within their current thermal niche as global temperatures rise. Unfortunately, movement across landscapes is made increasingly difficult for forest specialists by habitat loss and fragmentation.

![Forest fragmentation in Colombia. Photo credit: David Edwards.](colombia.JPG)

To calculate connectivity we directly adapted the methods in a great paper by @mcguire_achieving_2016, which focused on the USA. Everything is based on movement between forest patches, where a 'patch' is a continuous area of forest with the same temperature. For each patch we used some simple rules to determine the coldest other patch that can be reached by following a gradient of hotter to cooler adjacent patches. This is a bit like a game of draughts (aka checkers), where you can only move in one direction, one step at a time, and each move has particular rules that determine which of the various options is the best. Once you know the coldest patch that can be reached from your starting patch, you add in the projected future temperature under climate change. If the future temperature of the end patch is cooler (or the same) as the current temperature of the starting patch, then resident species could potentially avoid rising temperatures by moving.

When we do this for all forest in the tropics, the conclusions are...not good. In the year 2012, some 62% of tropical forest area failed to connect to anywhere cool enough to allow resident species to avoid climate warming by moving. Alarmingly, if species moved as far as possible to the coldest reachable forest, they would still experience 2.6&deg;C of warming by 2070. That's a lot for a region that has a characteristically stable climate over days, seasons and, indeed, millennia. 
