...
(Original post here: https://methodsblog.com/2019/08/21/thermal-images-r/#more-7506)
...
```{r fig-1, fig.cap= "The output of `plot_patches` includes a histogram and the original temperature data overlaid with outlines of hot and cold spots, identified using the G* variant of the Getis-Ord local statistic.", echo = TRUE}
plot_patches(
...
```{r fig-2, fig.cap= "Violin plots created using `plot_stack`. Each datapoint is a single pixel, from the 100 pixels sampled at random from each raster. Rasters derive from thermal images collected every 20 minutes in the same location.", echo = TRUE}
